...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("XAB://"))
    {
        uri.replace(0, 12, "XAB:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "XAB.lnk";
}
...
{
    return GetAutostartDir() / "XAB.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("XAB-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  XAB-qt [" + tr("command-line options") + "]                     " + "\n";
