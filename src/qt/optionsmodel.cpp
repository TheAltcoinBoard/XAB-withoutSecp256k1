...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXABAmount"))
        settings.setValue("nAnonymizeXABAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeXABAmount = settings.value("nAnonymizeXABAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeXABAmount"))
        SoftSetArg("-anonymizeXABamount", settings.value("nAnonymizeXABAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeXABAmount:
            nAnonymizeXABAmount = value.toInt();
            settings.setValue("nAnonymizeXABAmount", nAnonymizeXABAmount);
            emit anonymizeXABAmountChanged(nAnonymizeXABAmount);
            break;
        default:
            break;
        }
