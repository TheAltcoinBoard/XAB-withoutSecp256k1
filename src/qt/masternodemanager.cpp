...
void MasternodeManager::on_localButton_clicked()
{
    ...
    // Check if a local baXAB tree already exists
    BOOST_FOREACH(PAIRTYPE(std::string, CAdrenalineNodeConfig) adrenaline, pwalletMain->mapMyAdrenalineNodes)
    {
        if(adrenaline.second.isLocal)
	{
    ...
    if(pwalletMain->GetBalance() < 300000.1*COIN)
    {
	QMessageBox msg;
        msg.setText("You must have at least 300000.1 XAB to cover the 300000 XAB collateral for a Masternode and the tx fee.");
	msg.exec();
	return;
    }
    ...
    {
	QMessageBox msg;
        msg.setText("Your wallet must be unlocked so that the 300000 XAB collateral can be sent.");
	msg.exec();
	return;
    }
    ...
    string strError = pwalletMain->SendMoneyToDestination(CBitcoinAddress(account.vchPubKey.GetID()).Get(), 300000*COIN, sNarr, wtx);
    if (strError != "")
    {
	QMessageBox msg;
    ...
    {
	QMessageBox msg;
	std::string sMsg = "Local Masternode created and 300000 XAB sent to the collateral address.  Transaction hash:\n";
